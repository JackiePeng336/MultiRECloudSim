...
public class CloudletAssignmentPolicyBalance extends CloudletAssignmentPolicy {
	...
	public CloudletAssignmentPolicyBalance() { // 均衡任务分配器
	...
	//this function has no use in the class
	private double getCloudletPriroity(SimCloudlet c) {
		...
		priority += CPU_WEIGHT * c.getAverageMips() / REFERENCE_CPU;// + RAM_WEIGHT * c.getRam() / REFERENCE_RAM;
		//priority += IO_WEIGHT * c.getIo() / REFERENCE_IO + BW_WEIGHT * c.getBw() / REFERENCE_BW;
		return priority;
	}
	
	@Override
	public int[] assignCloudletsToVm(List<SimCloudlet> cloudletlist,
			List<Vm> vmlist) {
		addNewVmList(vmlist);
		...
		for (int i = 0; i < n; i++) {
			if (scList.get(i).getVmId() != -1)
				...
				//result[i] = scList.get(i).getVmId();
			else {
				int type = getCloudletIntensiveType(scList.get(i));
				int vmid = findMinLoadVmByType(type);
				assign.put(scList.get(i).getCloudletId(), vmid);
				addLoadForVm(VmList.getById(vmlist, vmid), scList.get(i));
				//Log.printLine("findMinLoadVmByType Type " + type + " result[i]" + result[i]);
				//printVmAllLoad();
				//addLoadForVm(VmList.getById(vmlist, result[i]),
				//		scList.get(i));
			}
		}
		for (int i = 0; i < n; i++) {
			//System.out.println("RB "+cloudletlist.get(i).getCloudletId());
	...
	public static int getCloudletIntensiveType(SimCloudlet cloudlet) { // 判断任务是哪种密集型
		...
		//Log.printLine("getCloudletIntensiveType "+type[CPU]+" "+type[RAM]+" "+type[IO]+" "+type[BW]);
		double max = 0.0;
		int maxType = 0;
		for (int i = 0; i < 4; i++) {
			if (type[i] > max) {
	...
	private void addLoadForVm(Vm vm, SimCloudlet cloudlet) { // 根据任务给虚拟机添加负载
		...
		float weight = (float) (cloudlet.getCloudletLength()/cloudlet.getAverageMips());// / cloudlet.getAverageMips());//cloudlet.getAverageMips());//NORMAL_LENGTH; // 长度是权重

		getCpuLoadIndicator().put(
				vm.getId(),
				(float) (cpuLoad + weight * cloudlet.getAverageMips()
						/ NORMAL_MIPS));

		getRamLoadIndicator().put(vm.getId(),
				(float) (ramLoad + weight * cloudlet.getRam() / NORMAL_RAM));

		getIoLoadIndicator().put(vm.getId(),
				(float) (ioLoad + weight * cloudlet.getIo() / NORAML_IO));

		getBwLoadIndicator().put(vm.getId(),
				(float) (bwLoad + weight * cloudlet.getBw() / NORMAL_BW));
	}
	...
	public void removeLoadForVm(Vm vm, SimCloudlet cloudlet) {// 根据任务给虚拟机减少负载
		...
		float ioLoad = getIoLoadForVm(vm);
		float bwLoad = getBwLoadForVm(vm);
		float weight = (float) (cloudlet.getCloudletLength() / cloudlet.getAverageMips());//cloudlet.getAverageMips()); // NORMAL_LENGTH;
		getCpuLoadIndicator().put(
				vm.getId(),
				(float) (cpuLoad - weight * cloudlet.getAverageMips()
						/ NORMAL_MIPS));
	...
	public void printVmAllLoad() {
		...
		for (Integer vmid : getCpuLoadIndicator().keySet()) {
			Log.printLine("Vm#" + vmid + " " + getCpuLoadIndicator().get(vmid)
					+ " " + getRamLoadIndicator().get(vmid) + " "
					+ getIoLoadIndicator().get(vmid) + " "
					+ getBwLoadIndicator().get(vmid));
		}
	}
